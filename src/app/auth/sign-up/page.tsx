...
import { Eye, EyeOff, Mail, Lock, ArrowRight, Github, User } from "lucide-react";
...
export default function SignUpPage() {
  ...
  const passwordStrength = password.length === 0 ? 0 : password.length < 6 ? 1 : password.length < 10 ? 2 : /[A-Z]/.test(password) && /\d/.test(password) ? 4 : 3;
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="hidden lg:flex lg:w-[480px] xl:w-[560px] flex-col justify-between bg-gradient-to-br from-accent via-primary/80 to-primary p-10 text-primary-foreground relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-6">
          <h1 className="text-3xl font-bold leading-tight">Start your<br />publishing journey<br />today.</h1>
          <p className="text-sm opacity-80 max-w-xs">
            From first draft to print-ready book — Kroj handles the typography so you can focus on the story.
          </p>

          <div className="space-y-3 pt-4">
            {["Professional typesetting engine", "12+ built-in book themes", "Export to ePub, PDF & DOCX", "Real-time collaboration"].map((feature) => (
        ...
        <div className="relative z-10">
          <p className="text-xs opacity-60">Free plan includes 5 books · No credit card required</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-sm space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Start writing your masterpiece</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              ...
              <div className="relative">
                ...
                <Input placeholder="Your full name" value={name} onChange={(e) => setName(e.target.value)} className="pl-9 h-10" />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input type={showPassword ? "text" : "password"} placeholder="Min. 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-9 pr-9 h-10" />
              ...
              {password.length > 0 && (
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <span className={`text-[10px] font-medium ${passwordStrength <= 1 ? "text-destructive" : passwordStrength === 2 ? "text-orange-500" : passwordStrength === 3 ? "text-yellow-600" : "text-green-600"}`}>{strengthLabels[passwordStrength]}</span>
                </div>
              )}
            </div>
            ...
            <div className="pt-1">
              <label className="flex items-start gap-2 text-xs text-muted-foreground cursor-pointer">
                <input type="checkbox" className="rounded border-muted-foreground/30 mt-0.5" />
